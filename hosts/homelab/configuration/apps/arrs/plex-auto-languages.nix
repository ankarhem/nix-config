{ config, pkgs, ... }:
let
  # copied from https://github.com/RemiRigal/Plex-Auto-Languages#configuration
  pal_config = {
    plexautolanguages = {
      ...
      plex = {
        ...
        url = "https://plex.internetfeno.men";
        # A valid Plex Token (required)
        # token = "MY_PLEX_TOKEN"; # Or set using environment variable PLEX_TOKEN
      };
      ...
      #     {
      #       urls = [ "gotify://hostname/token" "pover://user@token" ];
      #       users = [ "MyUser1" "MyUser2" ];
      #     }
      #
